/**
 * @file   test_avalon_rpm.c
 * @author Franz Luepke
 * @date   8 March 2021
 * @version 0.1
 * @brief  A Linux user space program that communicates with the avalon_rpm.c LKM. It passes a
 * int to the LKM and the PWM change with this number. For this example to work the device
 * must be called /dev/avalon_rpm_x.
*/
#include<stdio.h>
#include<stdlib.h>
#include<errno.h>
...
int main()
{
	...
	{
		char dev_path[BUFFER_LENGTH] = "/dev/avalon_rpm_";
		sprintf(buffer, "%d", i);
